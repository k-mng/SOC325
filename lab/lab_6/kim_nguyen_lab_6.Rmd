...
fit_processed <- textProcessor(documents = fit_df$comment,  stem = FALSE)

fit_out <- prepDocuments(documents = fit_processed$documents, vocab = fit_processed$vocab)
```
```{r cache = TRUE}
# Testing out STM functions
fit_stm5 <-  stm(documents = fit_out$documents, vocab = fit_out$vocab, K = 5, verbose = FALSE)
```

```{r, eval= FALSE, warning = FALSE}
# I couldn't run the 10, so I'm leaving it here to prove that I know what the code is
# fit_stm10 <- stm(documents = fit_out$documents, vocab = fit_out$vocab, K = 10, verbose = FALSE)
```

```{r}
# Summarizing the STM functions
par(mfrow=c(1,2))
plot(fit_stm5, n=7, text.cex = 0.7)
```

```{r}
# making the word cloud
cloud(fit_stm5, topic = 5)
```
