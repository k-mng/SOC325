...
```{r,message=FALSE,warning=FALSE}
df <- read.csv(here("data/lab_5/fitbit_data","IndividualSleepSummary.csv"))
...
Repeat the cleaning steps used in NOVEMBER
...
Print out out the `ndj` object using the `kbl()` function.
...
Plot the `ndj_clean` object similar to example analysis on NOVEMBER.
...
Perform the exploratory analysis on `ndj_clean` object similar to example analysis on NOVEMBER.
...
-   Bar Graph: There seems to be a bell-shape that forms with few outliers, which means that the average time that people sleep is around 8 hours
-   Scatter and Linear Relationship Graph: It looks as though there's no real pattern between the different days of the week. However, there is a clear upward trend the higher the sleep score.

Analysis:

-   I'm not exactly sure what the residuals and cor are supposed to stand for

-   Seems as though Saturday has the least in terms of sleep score, having an average of 80.69 while every other day is around the 85 level

-   Wednesday logs the least average hours of sleep at 8.2 hours, while the most (Friday) goes for 8.7 hours

It looks like that, despite the stereotypes of students and their sleep schedules, a lot people actually manage to get a good 8 hours of sleep!
